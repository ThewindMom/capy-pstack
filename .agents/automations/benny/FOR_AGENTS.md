...
i want two Capy automations that work together in one slack issue channel.
...
- i want the complete pstack skills installed in the target Capy project or selected skill volume only for shared dependencies such as `how`, `why`, `tdd`, `unslop`, and the required principle skills.
...
- budgets: `<event-driven triage deadline, follow-up, repro, rejection, fix>`
- optional bot token capability: `<none, file download, or editable operations status>`

start from [`configuration.example.yaml`](templates/configuration.example.yaml) and [`feature-map.example.md`](skills/reproduce-and-fix-issues/references/feature-map.example.md). copy and fill them outside this pack, for example under `.capy/benny/`. keep secret values in a secret manager or environment.

## for the agent

Read skills/setup-benny/SKILL.md directly. The full Capy-native pack and its shared
skills are already in this repository; use the offline installer to copy it into another
project or supported skill volume without discarding local changes. Keep user maps and
configuration outside the managed pack. Check a fresh task can read every dependency.

Create or update the two native Capy automations only after the user asks. Triage
normalizes Slack bursts into distinct root reports; repro consumes trusted triage-marker
events and validates their author and source coordinates. The complete operational
skills remain authoritative for classification, reproduction, two proofs, thread-only
posting, compensation and bounded draft fixes. Inspect existing automation IDs first,
create disabled, test thread safety and idempotent report receipts, then enable only the
authorized triggers. Never use an editor plugin registry or a detached wake loop.
