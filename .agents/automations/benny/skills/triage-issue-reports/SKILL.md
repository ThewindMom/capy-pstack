...
## Capy event normalization and receipts

Slack events may contain a burst of messages. Split distinct root reports before triage; preserve each root channel and thread timestamp. Process each root once in the owning queue. Save the triage task ID, verdict, and actual posted message ID in a scoped durable record. On retry, read the thread and reconcile any existing trusted marker before posting; a transport retry never authorizes a duplicate reply. Native run replies are not Slack posts: use the authorized explicit Slack action with the immutable thread coordinates.
