...
Accept only a trusted triage-marker event for the source thread. Reproduce the exact symptom through the target app's real UI. Verify an existing fix when one exists. Attempt a bounded fix only after a confirmed repro.
...
## 2. Validate the triage contract

Use the marker event to read the original source thread once. Do not poll or sleep waiting for another automation. If an older root-report trigger arrives before a marker, save it as pending with a deadline in the scoped state volume and return; an authorized native event or audit run may reconcile it. A missing wake capability remains a blocked pending workflow, not a fabricated scheduler.
...
Proceed only for `bug` or `performance`. Capture the optional tracker URL. Stop silently in Slack for `other`, a missing verdict, an untrusted author, conflicting markers, or an expired deadline; record the disposition in the run state. Atomically claim the immutable report key in the single owning queue before beginning work. A retry reconciles the previous task ID and published receipts instead of starting a second fixer.
...
Prefer authorized Capy Slack tools. Use `BENNY_SLACK_BOT_TOKEN` only when the user configured it for a narrow missing capability such as editing this one status message. Never expose the token to a worker.
...
- Use the configured temporary artifact directory while driving the app. Before the machine can be replaced or cleaned up, save the approved evidence to the selected durable volume or authorized attachment destination and verify it can be read there. Never commit captures or credentials.
