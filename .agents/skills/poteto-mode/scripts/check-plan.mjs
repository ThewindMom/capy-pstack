...
const PROGRAM_MARKERS = ["durable program goal", "git show origin/main:", /30[- ]minute/, "status message"];
...
for (const pr of prSections) {
	...
	if (live) {
		if (!LANES.test(live.rest)) fail(live.n, `${pr.title}: Verify, live lacks "${LANES}"`);
