#!/usr/bin/env python3
"""Validate Capy task plans and evidence. This tool does not create or bill tasks. Native Capy tasks are the lifecycle authority."""
...
def prepare(plan: dict) -> dict:
    ...
    require(type(plan.get("version")) is int and plan["version"] == 1, "Plan version must be 1")
    ...
    for task in by_id.values():
        ...
        if model not in ("inherit-parent", "auto"):
            task["model"] = model
        task["child_depth"] = depth + 1
        task["brief"] = (f"Read {root}/skills/poteto-mode/SKILL.md and the selected native playbook. "
