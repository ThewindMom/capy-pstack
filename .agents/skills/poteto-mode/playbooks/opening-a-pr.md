...
**Machine and branch.** The implementation owner works on a fresh Capy machine at the
accepted base, with an exclusive writable scope. Children do not inherit a parent's
working tree. Transfer required uncommitted inputs explicitly. Preserve unrelated work;
never reset or clean another owner's checkout to manufacture a fresh start.
...
**Publishing.** Prefer Capy's native PR tool, which commits, pushes and opens the PR
from the selected machine. Verify the returned branch, base, head and PR URL. Use the
available gh CLI only for an authorized operation the native tool cannot express. Native
PR ownership brings CI/review/merge wakeups; a manually published PR must be associated
with the intended owning thread through the available native controls before relying on
those events. Never invent subscription or SDK calls.
...
**Readiness.** Open every PR ready, never as a draft. With Origin, pass `--status open`. With `gh`, omit `--draft`. Set the actual native tool's readiness option explicitly and inspect the result; never assume its default. If a PR still opens as a draft, run `origin pr ready <number>` or `gh pr ready <number>` according to the resolved forge. Run `origin pr view <number>` or `gh pr view <number>` before you refer to PR status.
