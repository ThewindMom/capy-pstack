...
3. Explore in subagents as native Capy tasks with the complete poteto-agent role brief and the configured model or parent inheritance (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
- [ ] Use this tick prompt, verbatim. "Re-read the execution playbook from trunk and the saved program goal. Audit the operation against both and fix drift in this tick. Probe every active lane and judge progress by side effects only. Stop a stuck lane, confirm termination and reconcile its branch/scope before dispatching one replacement. Then post a status message to the operator in chat, whether or not anything changed, with the queue table of PR, owner, state, and head SHA, the verdicts since the last tick, what merged, open operator gates, and blockers."
