...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's actual Capy task messages and tool evidence using its native task ID. Use only this run's authorized history. A missing tool trace is missing evidence, not a passed chain-following gate. Look at which files each candidate actually opened. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
