...
- The brief is the product. A vague brief wastes a task; blocked children can notify the parent with a concrete question.

#### Roles and placement

- **Coordinator (this thread).** On the primary Capy machine. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through Capy's native task tools. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** A native task with a selected machine and persistent run record, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer. Each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nesting works to depth 3, and a nested spawn has the native task tool schema with explicit shared/fresh placement). Rolls up aggregates at wave boundaries. Never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window. Never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Writers use fresh Capy machines and accepted branches. Read-only verifiers may share a stable parent checkout; independent review of a writer uses a fresh machine at its actual head. Transfer inputs and evidence explicitly. Real UI/CLI checks run on the machine hosting the application. Device-only or unavailable credentials are explicit blocked gates. Keep one writer per scope and prefer observed different model families for independent judgment.
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape. A 4KB scaffold around a two-line edit costs more to write and obey than the edit. A shared-machine child may read the verified standing-orders path. Every fresh-machine task and every revision gets the full standing orders or an explicitly transferred copy.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget with explicit shared/fresh placement for each role, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).

A dependency is a context relay, not just ordering. Undeclared upstream context makes the worker guess. Missing fields are a refuse-to-spawn condition. Audit one sampled worker brief per sub-coordinator per wave, concurrently with the wave it samples, never as a gate in front of it. A failing brief stops that track and fixes the sub-coordinator's instructions, not just the worker, because brief quality decays late in a run. Do not rely on a chain of partial briefs. Send a consolidated brief to the existing owner. If replacement is necessary, confirm that owner is stopped and reconcile its scope and branch before one fresh task starts.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the actual native task state and messages. File mtime is not liveness.
...
- Before every replacement, confirm terminal failure or an explicit stop, reconcile the old machine and branch, and release its scope. Retry by mode: cap-hit or oom, start one fresh task with smaller scope. Network-drop, retry as-is. Tool-error, retry on a different model. Unknown, retry once. Two retries, then abandon the unit and replan around it.
...
- After a backing replacement, read the retained Capy thread/task state and the selected volume run record. Reconcile every native ID before replacing an owner; the machine identity can survive its backing. Recover only pushed branches or saved artifacts. Confirm no live lease owner before repairing a local ledger lock; a recycled PID or replaced machine is not proof that another owner is dead.
